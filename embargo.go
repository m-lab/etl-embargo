...
func ReadWhitelistFromGCS(path string) map[string]bool {
	...
	whiteList := make(map[string]bool)
	if fileContent, err := embargoService.Objects.Get("sidestream-embargo", path).Download(); err == nil {
...
func ShouldEmbargo(fileName string, whitelist map[string]bool) bool {
...
func EmbargoOneTar(content io.Reader, tarfileName string) bool {
	...
	for {
		...
		if strings.Contains(basename, "web100") && ShouldEmbargo(basename, whitelist) {
